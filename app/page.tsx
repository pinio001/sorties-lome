  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach((p) => {
        ...
        ctx.fillStyle = `hsla(${p.hue},80%,70%,${p.op})`; ctx.fill();
      });
...
function StatCard({ value, label, delay }: { value: number; label: string; delay: number }) {
  ...
  return (
    <div className="flex-1 text-center rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm py-4 px-2">
      <div className="text-2xl font-extrabold text-white tracking-tight">{count}+</div>
  ...
  return (
    <>
      <style>{`
        ...
        .live-dot { width:7px;height:7px;border-radius:50%;background:#22c55e;flex-shrink:0;animation:pulse 1.5s infinite; }
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-20 lg:items-center lg:min-h-[82vh]">
            ...
            <div className="max-w-md mx-auto lg:max-w-none lg:mx-0">
              ...
              <div className="fu3 mt-4">
                <h1 style={{ fontSize:"clamp(26px,3.5vw,54px)", fontWeight:700, lineHeight:1.15,
              ...
              <div className="fu4 mt-6 flex gap-3">
                <button onClick={() => router.push("/places")}
                  className="btn-primary flex-1 bg-white text-black font-semibold rounded-2xl py-3.5 transition"
                  style={{ fontSize:"clamp(13px,1vw,15px)" }}>
                  📍 Explorer Places
                </button>
                <button onClick={() => router.push("/events")}
                  className="btn-secondary flex-1 border border-white/20 text-white rounded-2xl py-3.5 transition"
                  style={{ backdropFilter:"blur(10px)", fontSize:"clamp(13px,1vw,15px)" }}>
                  🎉 Voir les Events
                </button>
              </div>
            ...
            <div className="fu6 mt-10 lg:mt-0">
              ...
              <div onClick={() => router.push("/events")}
                ...
                style={{ border:"1px solid rgba(59,130,246,.2)",
                  ...
                  backdropFilter:"blur(20px)", boxShadow:"0 0 40px rgba(59,130,246,.08)" }}>
                <div>
                  <div className="text-2xl lg:text-3xl mb-1">🎵</div>
                  <div style={{ fontSize:"clamp(15px,1.3vw,20px)", fontWeight:700, color:"#fff" }}>
                    Sorties & Events
                  </div>
                  <div className="text-xs lg:text-sm text-white/40 mt-1">Concerts · Clubs · Afterworks</div>
              ...
              <div onClick={() => router.push("/places")}
                ...
                style={{ border:"1px solid rgba(251,191,36,.2)",
                  background:"linear-gradient(135deg,rgba(251,191,36,.08) 0%,rgba(251,191,36,.02) 100%)",
                  backdropFilter:"blur(20px)", boxShadow:"0 0 40px rgba(251,191,36,.06)" }}>
                <div>
                  <div className="text-2xl lg:text-3xl mb-1">🏙️</div>
                  <div style={{ fontSize:"clamp(15px,1.3vw,20px)", fontWeight:700, color:"#fff" }}>
                    Bars, Restos & Plus
                  </div>
                  <div className="text-xs lg:text-sm text-white/40 mt-1">Bar/Resto · Hôtels · Clubs · Loisirs</div>
                  <div className="mt-3 inline-flex items-center text-xs px-2.5 py-1 rounded-lg"
